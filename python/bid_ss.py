...
print("\n")
log = "{:<55}\t {:>10}\t {:>8}\t {:>10}\t {:>8}\t {:>8}\t {:>9}\t"\
	.format("setting", "objective", "auction", "impression", "click", "cost", "win-rate")
print(log)
log_in.write(log + "\n")
...
for camp in camps:
	...
	rlb_dp_i = RLB_DP_I(camp_info, opt_obj, gamma, 1)
	...
	# RLB
	auction_in = open(data_path + camp + "/test.theta.txt", "r")
	rlb_dp_i = RLB_DP_I(camp_info, opt_obj, gamma, roi_threshold=1e-3)
	setting = "{}, camp={}, algo={}, N={}, c0={}" \
	...
	log = "{:<55}\t {:>10}\t {:>8}\t {:>10}\t {:>8}\t {:>8}\t {:>8.2f}" \
		.format(setting, obj, auction, imp, clk, cost, win_rate)
	print(log)
